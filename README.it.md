[![BCOS Certificato](https://img.shields.io/badge/BCOS-Certificato-brightgreen?style=flat)](BCOS.md)
...
- **Client del nodo**: controlli di stato, query del saldo, elenco dei miner
...
fn main() {
    ...
    // Firma un messaggio
    let sig = wallet.sign(b"ciao rustchain");
    let valid = Wallet::verify(&wallet.public_key_hex(), b"ciao rustchain", &sig).unwrap();
    ...
    println!("Nodo v{} (tempo attivo {}s)", health.version, health.uptime_s);
...
| Moderno | 1.0x | x86_64 attuale, aarch64 |
